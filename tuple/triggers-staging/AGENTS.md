# Tuple Triggers (staging)

This directory is the live trigger set for the staging Tuple client (deployed to `~/.tuplestaging/triggers` by `scripts/tuple-triggers-sync.sh` — the Tuple trigger runner does not follow symlinks, so the live dir holds real copies; edit here and run the sync script, or edit live and run it with `--capture`). Each subdirectory is one trigger; executables named for lifecycle events (`call-transcription-started`, `call-transcription-complete`, etc.) run when those events fire, receiving context via `TUPLE_TRIGGER_*` environment variables (notably `TUPLE_TRIGGER_CALL_ARTIFACTS_DIRECTORY`, the dated session folder under `~/Documents/Tuple Calls`).
