...
3. The agent reads the call via `tuple-staging transcription show <id> --with-events`, promotes durable facts into synthesis/entity pages with `date time \`callid\`` evidence pointers, lints links, appends a `Log.md` entry, and commits — then, best-effort, recommends Stephen's actions/projects into Fortress (each entry keyed on a unique `source_key: tuple-call:<id>:<slug>`, so retries upsert rather than duplicate).
