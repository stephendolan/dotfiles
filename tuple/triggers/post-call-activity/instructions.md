...
The recording lives in the daemon's DB store; read the call back through the Tuple CLI named in your task prompt (its official read interface):
...
- Verify who people are from the call's events (`tuple transcription show <id> --with-events --format json`) — names AND emails — never guess affiliations from conversation context alone. An email domain beats an inference (this rule exists because an agent once attributed participants to the wrong company from conversation context when their email domain said otherwise).
