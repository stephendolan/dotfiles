...
- **Two independent axes — transcript watch and screen watch.** How fast the sidekick samples the *talk* and how closely it shows *visible screen-frame receipts* are separate dials. Focused shared content changes also arrive as cheap text context — title, app, and URL when available — so Pi usually knows what is being shared without needing a screenshot. Pi controls both dials (`set_watch_mode`, `set_screen_watch`).
- **Adaptive cadence (`auto`, the default).** Instead of guessing up front, the sidekick opens in **real-time** for a short capture window (~30s) to hear how the call starts, then re-evaluates on the events that actually matter: **someone joins or leaves**, a **screen share starts or stops**, **focused shared content changes**, and a periodic check only after new speech (~5 min, which also catches topic drift like "let's try this in real time"). Each time, Pi judges what's happening and sets *both* axes — e.g. a teammate starts demoing → keep transcript quiet, rely on title/app/URL changes for semantic context, and use `set_screen_watch active` only if the user needs visible frame receipts. A new joiner gets a short settle so the conversation develops before the re-decision. Nudge the pace by hand (`/cadence` or the shortcuts) to take over; pick `auto` again to hand control back. Cadence and watch-word changes restart the in-flight transcript wait, so the new settings apply immediately instead of waiting for the old interval to return.
...
- **Inline screen captures.** When someone shares their screen, the sidekick can capture it **on its own** — the moment a share starts, then on a cadence set by the screen-watch level — and shows it inline as a visible receipt, *without needing an agent turn*. The level (`set_screen_watch`): **periodic** (a frame every few minutes — a light timeline for you) or **active** (a frame every ~15s for following a live demo), or **off**. Automatic frames are display-only and are not fed back into Pi's model context. Focused content changes are cheaper and usually enough for Pi: they flow into context as title/app/URL text, with URL query/hash stripped. Pi should use `capture_screen` only when visual details matter, which sends the current frame into Pi's context for inspection or a screen summary; `/screenshot` captures it yourself without model vision. A share that was already up before transcription started is detected too: the record stream only replays events from recording start, so at startup the sidekick probes once with `screen capture` (bytes back = someone is sharing) and seeds the share state from the result. Requires a graphics-capable terminal (Ghostty, iTerm2, Kitty, WezTerm).
...
Pi also has tools it can invoke itself as the call unfolds: `set_watch_mode` (transcript pace), `set_screen_watch` (how closely to show the shared screen — off/periodic/active), `post_summary` (muted topic summary), `record_artifact` (finding/fact-check/note), and `capture_screen` (one-off grab of the shared screen, sent into Pi's vision context and rendered inline for the user). Before every LLM call, the extension injects live call state (participants, elapsed, mode) via pi's `context` event, and names the session after the call so `pi -c` / `pi -r` can find it later. For heavier asks (deep research, thorny fact-checks), Pi can spin up a background subagent on request so it doesn't block the call.
...
| `screenshotActiveSec` | `15` | "active" screen-watch: seconds between display-only captures while following a demo. |
...
**Prod vs staging.** This copy targets the **production** Tuple client, so its `sidekick.config.json` sets `"cli": "tuple"` and the launcher defaults `TUPLE_CLI=tuple`. Both `tuple connect` and the extension's transcript/screen calls then hit the prod daemon. The staging copy (`tuple/triggers/staging/sidekick-pi`) sets `"cli": "tuple-staging"` instead — or set the `TUPLE_CLI` env var, which overrides the config. The launcher exports `SIDEKICK_PI_DIR` so the extension reliably finds its config regardless of Pi's module resolution.
