...
Panel {
  id: root
  moduleName: "stephen.agents"
  ipcTarget: "stephen.agents"
  manageIpc: false
  ...
  property string activeCodexProfile: "unknown"
  property bool codexProfileSwitching: false
  ...
  function readCodexProfile() {
    if (!codexProfileStatus.running) codexProfileStatus.running = true
  }

  function toggleCodexProfile() {
    if (codexProfileSwitch.running) return
    codexProfileSwitching = true
    codexProfileSwitch.running = true
  }
  ...
  function iconCandidatesForProvider(p, surfaceColor) {
    if (!p) return []
    var candidates = []
    var assetId = String(p.providerId || "").indexOf("codex-") === 0 ? "codex" : p.providerId
  ...
  Process {
    id: codexProfileStatus
    command: [Quickshell.env("HOME") + "/.local/bin/codex-profile", "status"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.activeCodexProfile = text.trim() || "unknown"
    }
  }

  Process {
    id: codexProfileSwitch
    command: [Quickshell.env("HOME") + "/.local/bin/codex-profile", "toggle"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") root.activeCodexProfile = text.trim()
    }
    onExited: function(exitCode) {
      root.codexProfileSwitching = false
      root.readCodexProfile()
    }
  }
      ...
      Flickable {
        ...
        Column {
          ...
          Row {
            ...
            Text {
              width: parent.width - codexProfileButton.width - parent.spacing
              anchors.verticalCenter: parent.verticalCenter
              text: "Codex default: " + (root.activeCodexProfile === "work" ? "Work" :
                    root.activeCodexProfile === "personal" ? "Personal" : "Unknown")
              color: root.dim
              font.family: root.fontFamily
            ...
            Button {
              id: codexProfileButton
              text: root.codexProfileSwitching ? "Switching…" :
                    root.activeCodexProfile === "work" ? "Use Personal" : "Use Work"
              enabled: !root.codexProfileSwitching && root.activeCodexProfile !== "unknown"
              bordered: true
              foreground: root.foreground
              fontFamily: root.fontFamily
              fontSize: Style.font.bodySmall
              verticalPadding: Style.spacing.controlPaddingY
              onClicked: root.toggleCodexProfile()
            }
          }
